...
for repo in configl.keys():
    if configl[repo]['service'] == 'gitlab':
        ...
        if '/gitbup/repos/{}'.format(gitreponame) in cronc:
            ...
            subprocess.run(['''echo "{} sh -c 'curl -fsS -m 10 --retry 5 -o /dev/null {}/start; cd /gitbup/repos/{}; git pull --all; curl --retry 3 {}/$?'" >> /var/spool/cron/crontabs/root'''.format(cronschedule, healthcheckurl, gitreponame, healthcheckurl)], shell=True) # 
